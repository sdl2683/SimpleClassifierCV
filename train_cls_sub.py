...
class Trainer():
    def __init__(self, model_name, num_classes, CFG, checkpoints_path=None):
        ...
        if self.scheduler == 'cos':
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=self.num_epoches)
        
    
    def train_model_kfold(self, model, bs, optimizer, scheduler, loss, skf, train_data, device):
        ...
        for fold_idx, (train_ids, val_ids) in enumerate(self.skf.split(df['img_path'], df['label'])):
            ...
            today = self.date
            if not ope(opj('train_data_file', today)):
                os.makedirs(today)
            ...
            train_dataset, val_dataset, test_dataset = getDataset(MyDataset=self.MyDataset, 
                                                                  ...
                                                                  get_train_transforms=self.get_train_transform, 
...
densenet121 = timm.create_model('densenet121',pretrained=True,num_classes=len(class2num.keys()))
densenet121.load_state_dict(torch.load('checkpoints/20230519/densenet121_20230519_epoch10_fold4_accuracy0.9877.pth'))

densenet161 = timm.create_model('densenet161',pretrained=True,num_classes=len(class2num.keys()))
densenet161.load_state_dict(torch.load('checkpoints/20230519/densenet161_20230519_epoch10_fold2_accuracy0.9907.pth'))
...
seresnext50.load_state_dict(torch.load('checkpoints/20230519/seresnext50_32x4d_20230519_epoch10_fold4_accuracy0.9895.pth'))

seresnext101 = timm.create_model('seresnext101_32x4d',pretrained=True,num_classes=len(class2num.keys()))
seresnext101.load_state_dict(torch.load('checkpoints/20230519/seresnext101_32x4d_20230519_epoch10_fold4_accuracy0.9642.pth'))
...
# model_list = [[densenet121, 24, 'cuda:0', 3e-6]]
# model_list = [[densenet161, 14, 'cuda:4', 5e-7]]


model_list = [[seresnext101, 16, 'cuda:0', 5e-6]]
# model_list = [[seresnext50, 24, 'cuda:1', 5e-7]]
